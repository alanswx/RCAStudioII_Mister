...
(
	...
	localparam int LEFT_GRID_W  = CELL_W;          // single "0" cell
...
`ifndef FAST_COMPILE2
	...
	// Any nudge of the left stick means "0" -- there is only one key on this row.
	...
	always @* begin : draw_overlay
		...
		if (show_left) begin
			...
			if ((pix_x >= box_x_i) && (pix_x < (box_x_i + BOX_W)) &&
				...
				// One "0" cell, kept in the centre column so it lines up with
				// the 2/5/8 column of the grid above it.
				for (idx_i = 1; idx_i < 2; idx_i = idx_i + 1) begin
					cell_x_i = BOX_PAD + (CELL_W + CELL_GAP);
					cell_y_i = BOX_PAD;
